...
interface WeddingMonthGroupsProps {
  ...
  /** When true, empty months still render (year browser). */
  showEmpty?: boolean;
}

export function WeddingMonthGroups({
  groups,
  showEmpty = false,
}: WeddingMonthGroupsProps) {
  const visible = showEmpty ? groups : groups.filter((g) => g.count > 0);
