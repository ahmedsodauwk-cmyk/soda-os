/**
 * Apply Smart Operations Engine migration.
 * Run: NODE_OPTIONS=--use-system-ca npx tsx scripts/apply-smart-ops-migration.ts
 */
...
const MIGRATION = "supabase/migrations/20260712000008_smart_ops.sql";
...
async function main() {
  ...
  console.log("=== Apply Smart Ops migration ===");
...
`);
  ...
  try {
    ...
    console.log(`
If storage.buckets is unavailable, paste SODA_SMART_OPS.sql in the Dashboard
and skip/ignore the storage insert, or create bucket "soda-files" manually.
`);
    process.exit(1);
  }
}
